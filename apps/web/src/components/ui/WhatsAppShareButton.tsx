...
export function WhatsAppShareButton({ title, url, className }: Props) {
  ...
  const text = encodeURIComponent(`${title} — ${t('shareIntro')}: ${url}`)
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-1.5 font-display text-kicker uppercase tracking-[0.1em] font-semibold text-ink-muted hover:text-bersama-blue transition-colors ${
        className ?? ''
      }`}
    >
